<template>
  <div>
    ...
    <h4 style="color:red">下方人員列表的總人數是{{personList.length }}</h4>
    ...
    <button @click="plusOdd(count)">當總和為奇數才加</button>
    <button @click="plusWait(count)">等一等再加</button>
  </div>
</template>
...
export default {
    ...
    methods:{
        ...mapMutations('testAbout',{plus:'PLUS',minus:'MINUS'}),
        ...mapActions('testAbout',['plusOdd','plusWait'])
        // plus(){
    ...
    computed:{
        ...mapState('testAbout',['sum','studentName','studentAge']),
        ...mapState('personAbout',['personList']),
        ...mapGetters('testAbout',['bigSum'])
    }
}
</script>
